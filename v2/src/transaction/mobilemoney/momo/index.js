...
const {
  getUserId, getUserDetails, getTargetCountry, number_format, getExchangeRate,
...
export const transactionDepositMomo = async (req, res) => {
  try {
    ...
    if (_depositorIsValidZMPhoneNumber) {
      ...
      const depositorstatusresult = await lib.checkIfRecipientExists(depositorId);
      ...
      const escrowprivkey = await lib.getSenderPrivateKey(escrowInfo.data().seedKey, escrowMSISDN, iv);
      ...
      const receipt = await sendcUSD(escrowInfo.data().publicAddress, depositInfo.data().publicAddress, `${cusdAmount}`, escrowprivkey);
...
export const transactionWithdrawMomo = async (req, res) => {
  try {
    ...
    const withdrawerstatusresult = await lib.checkIfRecipientExists(withdrawerId);
    ...
    // Retrieve User Blockchain Data
    const escrowInfo = await lib.getReceiverDetails(escrowId);
    const withdrawerprivkey = await lib.getSenderPrivateKey(withdrawerInfo.data().seedKey, withdrawMSISDN, iv);
    let cusdAmount = number_format(amount, 4);
    const ghs_to_usd = await getExchangeRate(GHS_TO_USD);
    cusdAmount *= ghs_to_usd;
    cusdAmount = number_format(cusdAmount, 4);
    const cusdtoken = await kit.contracts.getStableToken();
    const cusdBalance = await cusdtoken.balanceOf(withdrawerInfo.data().publicAddress);
    ...
    const receipt = await sendcUSD(withdrawerInfo.data().publicAddress, escrowInfo.data().publicAddress, `${cusdAmount}`, withdrawerprivkey);
    const url = await getTxidUrl(receipt.transactionHash);
