...
export const transactionsGetEscrow = async (req, res) => {
  ...
  try {
    ...
    if (escrowMSISDN == null) {
      return res.json({ status: 400, desc: 'country not supported' });
    }
    ...
    res.json({
      ...
      conversionRate: {
        ...
        cusdToLocalCurrencyRate: `${cusd2localCurrencyRate}`,
      },
    });
  } catch (e) {
    // console.log(e);
    res.json({ status: 400, desc: 'invalid request', error: e });
  }
};
...
export const transactionsTransferP2p = async (req, res) => {
  try {
    ...
    const _isValidSender = await isValidPhoneNumber(
      senderMSISDN,
      targetCountry
    );

    const receiverMSISDN = await validateMSISDN(recipient, targetCountry);
    const _isValidRecipient = await isValidPhoneNumber(
      receiverMSISDN,
      targetCountry
    );
    ...
    if (_isValidSender && _isValidRecipient) {
      ...
      if (!isSenderVerified) {
        return res.json({ status: 400, desc: 'not verified' });
      }
      ...
      const recipientstatusresult = await lib.checkIfRecipientExists(
        recipientId
      );
      if (!recipientstatusresult) {
        return res.json({ status: 400, desc: 'user account does not exist' });
      }

      let senderInfo = await lib.getUserDetails(senderId);
      while (
        senderInfo.data() === undefined
        || userInfo.data() === null
        || userInfo.data() === ''
      ) {
        await sleep(1000);
    ...
    const receipt = await sendcUSD(
      recipientInfo.data().publicAddress,
