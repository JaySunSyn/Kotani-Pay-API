...
const resetPin = async (req, res) => {
  try {
    ...
    if (_isValidPhoneNumber) {
      ...
      if (!isKyced) {
        return res.json({ status: 400, desc: "User is not KYC'ed" });
      }
      ...
      res.json({
        status: 201,
        desc: `${userMSISDN} Kotani Pay PIN updated successfully`,
      });
    }
  } catch (e) {
...
const getBalance = async (req, res) => {
  ...
  try {
    ...
    const { permissionLevel } = req.user;
    if (permissionLevel != 'partner' && permissionLevel != 'admin') {
