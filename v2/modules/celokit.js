...
exports.getTxAmountFromHash = async (hash) => {
  const _tx = await kit.web3.eth.getTransaction(hash);
  const amount = await weiToDecimal(_tx.value);
  console.log(amount);
  return amount;
};
...
const getTransactionBlock = async (txhash) => {
  const _res = await kit.web3.eth.getTransaction(txhash);
...
exports.sendcUSD = async (sender, receiver, cusdAmount, privatekey) => {
  ...
  const cusdbalance = await cusdtoken.balanceOf(sender); // In cUSD
...
exports.heckUbiScBalance = async (UBISCADDRESS) => {
  try {
    ...
    const balanceOf = await stableToken.methods.balanceOf(UBISCADDRESS).call();
...
// working
exports.getLatestBlock = async () => await kit.web3.eth.getBlock('latest');
...
exports.getWithdrawerBalance = async (publicAddress) => {
  ...
  const _cusdbalance = await weiToDecimal(cusdbalance);
  console.info(`Account balance of ${_cusdbalance} CUSD`);
  console.log('Numeric Balance: ', cusdBalance);
  return parseFloat(_cusdbalance).toFixed(4);
};
