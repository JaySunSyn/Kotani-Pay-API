...
const getReceiveFromEasypaySignature = async (
  trxReference,
  trxAmount,
  merchantCode,
  countryCode
) => {
  const jenga_trx_reference_id =
    trxReference + trxAmount + merchantCode + countryCode;
...
exports.receiveFromEazzypayPush = async (
  mobileNumber,
  referenceCode,
  amount
) => {
  try {
    ...
    let signature = await getReceiveFromEasypaySignature(
      referenceCode,
      amount,
      merchant_code,
      country_code
...
exports.sendFromJengaToMobileMoney = async (
  amount,
  referenceCode,
  currencyCode,
  countryCode,
  ...
  try {
    let access_token = await getAccessToken();
    let signature = await getSendToUserMobileSignature(
      amount,
      currencyCode,
      referenceCode
    );
...
exports.getUserKyc = async (
  ...
  let access_token = await getAccessToken();
  // console.log('Access Token: ',access_token);
  let signature = await getKycSignature(
    merchantcode,
    documentNumber,
    countryCode
  );
